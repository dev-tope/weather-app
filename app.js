...
const getWeatherData = async (city) => {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/current.json?key=99e6deaff0e045e3903111056232712&q=${city}&aqi=no`
    );
    if (!response.ok) {
...
async function renderDefaultData() {
  try {
    const responseIP = await fetch(
      `https://api.weatherapi.com/v1/ip.json?key=99e6deaff0e045e3903111056232712&q=auto:ip&aqi=no`
    );
    if (!responseIP.ok) {
...
async function getForecastData(city) {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=99e6deaff0e045e3903111056232712&q=${city}&days=4&aqi=no&alerts=no`
    );
    if (!response.ok) {
      throw new Error(response.status);
    }
...
function getForecastInfo(data) {
  for (let i = 0; i <= 2; i++) {
    ...
    forecastArr[i].tempC = Math.round(
      data.forecast.forecastday[i].day.avgtemp_c
    );
    forecastArr[i].tempF = Math.round(
...
function renderTemp(data, forecast) {
  if (currentState === "f") {
    temp.innerText = `${data.tempF}°F`;
    forecastTemp1.innerText = `${forecast[1].tempF}°F`;
    forecastTemp2.innerText = `${forecast[2].tempF}°F`;
  }
  if (currentState === "c") {
    temp.innerText = `${data.tempC}°C`;
    forecastTemp1.innerText = `${forecast[1].tempC}°C`;
...
toggleBtn.addEventListener("click", () => {
  if (currentState === "c") {
    currentState = "f";
    renderTemp(weatherData, forecastData);
    symbol.innerText = "°C";
  } else {
    currentState = "c";
    renderTemp(weatherData, forecastData);
